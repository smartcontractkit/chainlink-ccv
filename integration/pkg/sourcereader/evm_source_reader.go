...
func (r *EVMSourceReader) FetchMessageSentEvents(ctx context.Context, fromBlock, toBlock *big.Int) ([]protocol.MessageSentEvent, error) {
	...
	for _, log := range logs {
		...
		// Convert OnRamp receipts to protocol format and validate structure
		numCCVBlobs := len(event.VerifierBlobs)
		// Determine number of token receipts: 0 if no token transfer, 1 if token transfer exists.
		// CCIP v1.7 only supports 0 or 1 token transfer per message.
		numTokenTransfers := 0
		if decodedMsg.TokenTransferLength > 0 {
			numTokenTransfers = 1
		}
		...
		if err != nil {
			r.lggr.Errorw("Failed to parse receipt structure",
				"error", err,
				"receiptsCount", len(event.Receipts),
				"ccvBlobsCount", numCCVBlobs,
				"tokenTransfersCount", numTokenTransfers)
			continue // to next message
		}

		r.lggr.Infow("Receipt structure validated",
			"ccvCount", len(receiptStructure.CCVReceipts),
			"tokenCount", len(receiptStructure.TokenReceipts),
			"executorAddress", receiptStructure.ExecutorAddress.String())
