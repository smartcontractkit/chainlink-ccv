...
// tell a conversion from a standalone file. LoadConfigFile reports the conversion itself.
...
`), 0o600))
	...
	cfg, conversion, err := LoadConfigFile(path)
	require.NoError(t, err)
...
`), 0o600))
	...
	_, conversion, err := LoadConfigFile(path)
	require.NoError(t, err)
