...
func TestSyncJobProposals_StatusChange_UpdatesDatastore(t *testing.T) {
	...
	err := deployments.SaveJob(ds, localJob)
	require.NoError(t, err)
	...
	updatedJob, err := deployments.GetJob(report.Output.DataStore.Seal(), "nop-1", "nop-1-default-executor")
	require.NoError(t, err)
...
func TestSyncJobProposals_JobNotFoundInJD_MarkedAsOrphanedAndDeleted(t *testing.T) {
	...
	_, err = deployments.GetJob(report.Output.DataStore.Seal(), "nop-1", "nop-1-default-executor")
	require.Error(t, err)
}
...
func TestSyncJobProposals_NOPFilter_OnlySyncsFilteredNOPs(t *testing.T) {
	...
	require.NoError(t, deployments.SaveJob(ds, job1))
	require.NoError(t, deployments.SaveJob(ds, job2))
