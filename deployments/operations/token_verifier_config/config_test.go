...
import (
	...
	"github.com/smartcontractkit/chainlink-ccv/deployments/operations/token_verifier_config"
...
func TestBuildConfig(t *testing.T) {
	...
	// Note: operations.NewOperation returns an operation that wraps the function
	// For testing, we recreate the call inline
	...
	for _, chainSelector := range input.ChainSelectors {
		...
		cctpVerifierAddr, err := dsutil.FindAndFormatRef(ds2, datastore.AddressRef{
			Type:      datastore.ContractType(cctp_verifier.ContractType),
		...
		cctpVerifierResolverAddr, err := dsutil.FindAndFormatRef(ds2, datastore.AddressRef{
			Type:      datastore.ContractType(cctp_verifier.ResolverType),
			Qualifier: input.CCTPQualifier,
		}, chainSelector, toAddress)
		require.NoError(t, err)
		...
		lombardVerifierResolverAddr, err := dsutil.FindAndFormatRef(ds2, datastore.AddressRef{
			Type:      datastore.ContractType(lombard_verifier.ResolverType),
