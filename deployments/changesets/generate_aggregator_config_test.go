...
func TestGenerateAggregatorConfig_GeneratesCorrectConfigFromOnChainState(t *testing.T) {
	...
	cfg, err := deployments.GetAggregatorConfig(output.DataStore.Seal(), "test-aggregator")
	require.NoError(t, err)
	...
	expectedConfig := &model.Committee{
...
func toModelSigners(addrs []common.Address) []model.Signer {
	signers := make([]model.Signer, 0, len(addrs))
	for _, addr := range addrs {
		signers = append(signers, model.Signer{Address: addr.Hex()})
	}
	return signers
}
