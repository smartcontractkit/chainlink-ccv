...
func TestApplyExecutorConfig_PreservesExistingJobSpecs(t *testing.T) {
	...
	_, err = deployments.GetJob(outputSealed, "nop-1", "nop-1-default-executor")
	require.NoError(t, err, "new executor job spec should be present")

	retrievedJob, err := deployments.GetJob(outputSealed, "existing-nop", "existing-verifier")
...
func TestApplyExecutorConfig_RemovesOrphanedJobSpecs(t *testing.T) {
	...
	_, err = deployments.GetJob(outputSealed, "removed-nop", "removed-nop-default-executor")
...
func TestApplyExecutorConfig_UpdatesJobsWhenChainRemoved(t *testing.T) {
	...
	firstJob, err := deployments.GetJob(firstOutput.DataStore.Seal(), "nop-1", "nop-1-default-executor")
	require.NoError(t, err)
	...
	secondJob, err := deployments.GetJob(secondOutput.DataStore.Seal(), "nop-1", "nop-1-default-executor")
	require.NoError(t, err)
...
func TestApplyExecutorConfig_AllNOPsUpdatedWhenMemberAddedToPool(t *testing.T) {
	...
	nop1Job, err := deployments.GetJob(firstSealedDS, "nop-1", "nop-1-default-executor")
	require.NoError(t, err)
	nop2Job, err := deployments.GetJob(firstSealedDS, "nop-2", "nop-2-default-executor")
	require.NoError(t, err)
	...
	nop1JobUpdated, err := deployments.GetJob(secondSealedDS, "nop-1", "nop-1-default-executor")
	require.NoError(t, err)
	nop2JobUpdated, err := deployments.GetJob(secondSealedDS, "nop-2", "nop-2-default-executor")
	require.NoError(t, err)
	nop3Job, err := deployments.GetJob(secondSealedDS, "nop-3", "nop-3-default-executor")
	require.NoError(t, err)
...
func TestApplyExecutorConfig_AllNOPsUpdatedWhenMemberRemovedFromPool(t *testing.T) {
	...
	for _, nop := range []shared.NOPAlias{"nop-1", "nop-2", "nop-3"} {
		job, err := deployments.GetJob(firstSealedDS, nop, shared.JobID(string(nop)+"-default-executor"))
		require.NoError(t, err)
