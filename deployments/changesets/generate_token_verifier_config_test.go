...
func TestGenerateTokenVerifierConfig_GeneratesConfigWithBothVerifiers(t *testing.T) {
	...
	var cctpVerifier, lombardVerifier *tokenverifier.VerifierConfig
...
func TestGenerateTokenVerifierConfig_PreservesExistingConfigs(t *testing.T) {
	...
	existingIndexerConfig := &idxconfig.GeneratedConfig{
		Verifier: []idxconfig.GeneratedVerifierConfig{
			{
				Name:            "test-verifier",
	...
	_, err = deployments.GetTokenVerifierConfig(outputSealed, testServiceIdentifier)
	require.NoError(t, err)
