...
func (m *Manager) handleProposal(proposal *pb.ProposeJobRequest) (retErr error) {
	...
	if err := m.startJob(proposal.Spec); err != nil {
		if wasRunning {
			...
			rollbackCtx, rollbackCancel := context.WithTimeout(context.Background(), m.controlPlaneTimeout)
			defer rollbackCancel()
			return m.rollbackReplacement(
				rollbackCtx,
				proposal.Id,
				fmt.Errorf("failed to start replacement job: %w", err),
				currentJob,
			)
		}
...
func (m *Manager) rollbackReplacement(ctx context.Context, newProposalID string, cause error, oldJob *store.Job) error {
	metricsCtx := context.Background()
	var errs error
