...
type Phase1Component interface {
	RunPhase1(ctx context.Context, globalConfig map[string]any, componentConfig any) (map[string]any, []Effect, error)
}

// Phase2Component runs during Phase 2 (protocol platform deployments).
type Phase2Component interface {
	RunPhase2(ctx context.Context, globalConfig map[string]any, componentConfig any, priorOutputs map[string]any) (map[string]any, []Effect, error)
}

// Phase3Component runs during Phase 3 (CCVs and token pools).
type Phase3Component interface {
	RunPhase3(ctx context.Context, globalConfig map[string]any, componentConfig any, priorOutputs map[string]any) (map[string]any, []Effect, error)
}
