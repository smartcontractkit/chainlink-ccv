...
func (a *AggregatorInput) GenerateConfig(inV []*VerifierInput) (tomlConfig []byte, thresholdPerSource map[uint64]uint8, err error) {
	...
	for _, v := range inV {
		...
		if v.EnabledChains == nil {
			...
			for _, chainID := range v.EnabledChains {
				...
				if err != nil {
					return nil, nil, fmt.Errorf("failed to get chain details for chain ID %d: %w", chainID, err)
				}
	...
	for chainSelector, verifierAddress := range a.CommitteeVerifierResolverAddresses {
		...
		if a.ThresholdPerSource != nil {
			...
			if err != nil {
				return nil, nil, fmt.Errorf("failed to get chain ID from chain selector %d: %w", chainSelector, err)
			}
	...
	fmt.Printf("Generated aggregator config:\n%s\n", string(cfg))
	if err != nil {
