...
func TestE2ESmoke(t *testing.T) {
	...
	t.Run("extra args v2", func(t *testing.T) {
		tcs := []v2TestCase{
			...
			},
			{
				...
				fromSelector:             sel0,
				toSelector:               sel2,
				receiver:                 getContractAddress(t, in, sel2, datastore.ContractType(mock_receiver.ContractType), mock_receiver.Deploy.Version(), devenvcommon.DefaultReceiverQualifier, "mock receiver"),
				expectFail:               false,
				numExpectedVerifications: 1,
			},
		}
	...
	t.Run("extra args v3 messaging", func(t *testing.T) {
		...
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.aggregatorQualifier != "" && tc.aggregatorQualifier != devenvcommon.DefaultCommitteeVerifierQualifier {
	...
	t.Run("extra args v3 token transfer", func(t *testing.T) {
		...
		runTokenTransferTestCase := func(t *testing.T, combo devenvcommon.TokenCombination, finalityConfig uint16, receiver protocol.UnknownAddress) {
			...
			messageOptions := cciptestinterfaces.MessageOptions{
				...
				Executor:          getContractAddress(t, in, sel0, datastore.ContractType(executor.ProxyType), executor.DeployProxy.Version(), devenvcommon.DefaultExecutorQualifier, "executor"),
			}
		...
		for _, combo := range devenvcommon.All17TokenCombinations() {
			receiver := mustGetEOAReceiverAddress(t, destChain)
	...
	t.Run("USDC v3 token transfer", func(t *testing.T) {
		var (
			...
			cctpOnlyReciver = getContractAddress(
				...
				datastore.ContractType(mock_receiver.ContractType),
				mock_receiver.Deploy.Version(),
				devenvcommon.CCTPPrimaryReceiverQualifier,
				"",
			)
			cctpAndCommiteeReciver = getContractAddress(
				...
				datastore.ContractType(mock_receiver.ContractType),
				mock_receiver.Deploy.Version(),
				devenvcommon.CCTPSecondaryReceiverQualifier,
				"",
			)
		)
		...
		runUSDCTestCase := func(
			...
			srcToken := getTokenAddress(t, in, sourceSelector, devenvcommon.CCTPContractsQualifier)
			destToken := getTokenAddress(t, in, destSelector, devenvcommon.CCTPContractsQualifier)

			startBal, err := destChain.GetTokenBalance(ctx, tc.receiver, destToken)
			require.NoError(t, err)
			l.Info().Str("Receiver", tc.receiver.String()).Uint64("StartBalance", startBal.Uint64()).Str("Token", devenvcommon.CCTPContractsQualifier).Msg("receiver start balance")

			srcStartBal, err := sourceChain.GetTokenBalance(ctx, sender, srcToken)
			require.NoError(t, err)
			l.Info().Str("Sender", sender.String()).Uint64("SrcStartBalance", srcStartBal.Uint64()).Str("Token", devenvcommon.CCTPContractsQualifier).Msg("sender start balance")

			seqNo, err := sourceChain.GetExpectedNextSequenceNumber(ctx, destSelector)
			require.NoError(t, err)
			l.Info().Uint64("SeqNo", seqNo).Str("Token", devenvcommon.CCTPContractsQualifier).Msg("expecting sequence number")
			...
			require.NoError(t, err)
			require.NotNil(t, sendRes)
			require.Len(t, sendRes.ReceiptIssuers, tc.expectedReceiptIssuers, "expected %d receipt issuers for %s token", tc.expectedReceiptIssuers, devenvcommon.CCTPContractsQualifier)
			...
			l.Info().Uint64("EndBalance", endBal.Uint64()).Str("Token", devenvcommon.CCTPContractsQualifier).Msg("receiver end balance")

			srcEndBal, err := sourceChain.GetTokenBalance(ctx, sender, srcToken)
			require.NoError(t, err)
			require.Equal(t, new(big.Int).Sub(new(big.Int).Set(srcStartBal), tc.transferAmount), srcEndBal)
			l.Info().Uint64("SrcEndBalance", srcEndBal.Uint64()).Str("Token", devenvcommon.CCTPContractsQualifier).Msg("sender end balance")
		}
...
func customExecutorTestCase(t *testing.T, src, dest uint64, in *ccv.Cfg) v3TestCase {
	return v3TestCase{
		...
		ccvs: []protocol.CCV{
			{
				CCVAddress: getContractAddress(t, in, src, datastore.ContractType(committee_verifier.ResolverType), committee_verifier.Deploy.Version(), devenvcommon.DefaultCommitteeVerifierQualifier, "committee verifier proxy"),
				Args:       []byte{},
				ArgsLen:    0,
			},
		},
		...
		expectFail:               false,
		numExpectedVerifications: 1,
		executor:                 getContractAddress(t, in, src, datastore.ContractType(executor.ProxyType), executor.DeployProxy.Version(), devenvcommon.CustomExecutorQualifier, "executor"),
	}
}
	...
	return []v3TestCase{
		{
			...
			expectFail:               false,
			numExpectedVerifications: 1,
			executor:                 getContractAddress(t, in, src, datastore.ContractType(executor.ProxyType), executor.DeployProxy.Version(), devenvcommon.DefaultExecutorQualifier, "executor"),
		},
	}
}
	...
	return []v3TestCase{
		...
		},
		{
			...
			receiver: getContractAddress(
				...
				datastore.ContractType(mock_receiver.ContractType),
				mock_receiver.Deploy.Version(),
				devenvcommon.SecondaryReceiverQualifier,
			...
			numExpectedReceipts: 3,
			executor:            getContractAddress(t, in, src, datastore.ContractType(executor.ProxyType), executor.DeployProxy.Version(), devenvcommon.DefaultExecutorQualifier, "executor"),
			aggregatorQualifier: devenvcommon.SecondaryCommitteeVerifierQualifier,
		},
		{
			...
			dstSelector: dest,
			finality:    1,
			receiver:    getContractAddress(t, in, dest, datastore.ContractType(mock_receiver.ContractType), mock_receiver.Deploy.Version(), devenvcommon.SecondaryReceiverQualifier, "secondary mock receiver"),
			ccvs: []protocol.CCV{
		...
		},
		{
			...
			dstSelector: dest,
			finality:    1,
			receiver:    getContractAddress(t, in, dest, datastore.ContractType(mock_receiver.ContractType), mock_receiver.Deploy.Version(), devenvcommon.QuaternaryReceiverQualifier, "quaternary mock receiver"),
			ccvs: []protocol.CCV{
