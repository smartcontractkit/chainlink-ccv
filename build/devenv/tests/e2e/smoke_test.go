...
func TestE2ESmoke(t *testing.T) {
	...
	t.Run("extra args v2", func(t *testing.T) {
		tcs := []struct {
		...
		}{
			{
				...
				fromSelector:             sel[0],
				toSelector:               sel[1],
				receiver:                 mustGetEOAReceiverAddress(t, chains[sel[1]], sel[1]),
				expectFail:               false,
			...
			},
			{
				...
				fromSelector:             sel[1],
				toSelector:               sel[0],
				receiver:                 mustGetEOAReceiverAddress(t, chains[sel[0]], sel[0]),
				expectFail:               false,
				numExpectedVerifications: 1,
			},
			{
				...
				fromSelector:             sel[0],
				toSelector:               sel[2],
				receiver:                 getContractAddress(t, in, sel[2], datastore.ContractType(mock_receiver.ContractType), mock_receiver.Deploy.Version(), evm.DefaultReceiverQualifier, "mock receiver"),
				expectFail:               false,
				numExpectedVerifications: 1,
			},
		}
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				seqNo, err := chains[tc.fromSelector].GetExpectedNextSequenceNumber(ctx, tc.fromSelector, tc.toSelector)
				require.NoError(t, err)
				l.Info().Uint64("SeqNo", seqNo).Msg("Expecting sequence number")
				_, err = chains[tc.fromSelector].SendMessage(ctx, tc.fromSelector, tc.toSelector, cciptestinterfaces.MessageFields{
				...
				e, err := chains[tc.toSelector].WaitOneExecEventBySeqNo(ctx, tc.fromSelector, tc.toSelector, seqNo, defaultExecTimeout)
				require.NoError(t, err)
				require.NotNil(t, e)
		...
		for _, tc := range tcs {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.tokenTransfer != nil {
					...
					receiverStartBalance, err = chains[tc.dstSelector].GetTokenBalance(ctx, tc.dstSelector, tc.receiver, destTokenAddress)
					require.NoError(t, err)
				...
				seqNo, err := chains[tc.srcSelector].GetExpectedNextSequenceNumber(ctx, tc.srcSelector, tc.dstSelector)
				require.NoError(t, err)
				l.Info().Uint64("SeqNo", seqNo).Msg("Expecting sequence number")
				sendMessageResult, err := chains[tc.srcSelector].SendMessage(
				...
				sentEvent, err := chains[tc.srcSelector].WaitOneSentEventBySeqNo(ctx, tc.srcSelector, tc.dstSelector, seqNo, defaultSentTimeout)
				require.NoError(t, err)
				messageID := sentEvent.MessageID
				...
				e, err := chains[tc.dstSelector].WaitOneExecEventBySeqNo(ctx, tc.srcSelector, tc.dstSelector, seqNo, defaultExecTimeout)
				require.NoError(t, err)
				require.NotNil(t, e)
				if tc.expectFail {
				...
				if receiverStartBalance != nil {
					receiverEndBalance, err := chains[tc.dstSelector].GetTokenBalance(ctx, tc.dstSelector, tc.receiver, destTokenAddress)
					require.NoError(t, err)
	...
	t.Run("extra args v3 token transfer", func(t *testing.T) {
		runTokenTransferTestCase := func(t *testing.T, combo evm.TokenCombination, finalityConfig uint16, receiver protocol.UnknownAddress) {
			sender := mustGetSenderAddress(t, chains[sel[0]], sel[0])

			srcToken := getTokenAddress(t, in, sel[0], combo.SourcePoolAddressRef().Qualifier)
			destToken := getTokenAddress(t, in, sel[1], combo.DestPoolAddressRef().Qualifier)

			startBal, err := chains[sel[1]].GetTokenBalance(ctx, sel[1], receiver, destToken)
			require.NoError(t, err)
			...
			srcStartBal, err := chains[sel[0]].GetTokenBalance(ctx, sel[0], sender, srcToken)
			require.NoError(t, err)
			...
			seqNo, err := chains[sel[0]].GetExpectedNextSequenceNumber(ctx, sel[0], sel[1])
			require.NoError(t, err)
			...
			messageOptions := cciptestinterfaces.MessageOptions{
				...
				Executor:          getContractAddress(t, in, sel[0], datastore.ContractType(executor.ContractType), executor.Deploy.Version(), "", "executor"),
			}

			sendRes, err := chains[sel[0]].SendMessage(
				ctx, sel[0], sel[1],
			...
			sentEvt, err := chains[sel[0]].WaitOneSentEventBySeqNo(ctx, sel[0], sel[1], seqNo, defaultSentTimeout)
			require.NoError(t, err)
			...
			execEvt, err := chains[sel[1]].WaitOneExecEventBySeqNo(ctx, sel[0], sel[1], seqNo, 45*time.Second)
			require.NoError(t, err)
			...
			endBal, err := chains[sel[1]].GetTokenBalance(ctx, sel[1], receiver, destToken)
			require.NoError(t, err)
			...
			srcEndBal, err := chains[sel[0]].GetTokenBalance(ctx, sel[0], sender, srcToken)
			require.NoError(t, err)
...
func mustGetSenderAddress(t *testing.T, c cciptestinterfaces.CCIP17ProductConfiguration, chainSelector uint64) protocol.UnknownAddress {
	...
	return []testcase{
		{
			...
			dstSelector: dest,
			finality:    1,
			receiver:    mustGetEOAReceiverAddress(t, c[dest], dest),
			ccvs: []protocol.CCV{
				{
