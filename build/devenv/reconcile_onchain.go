...
// Parity with deploy: lane assembly uses buildConnectionEntriesFromImpls + buildPartialChainConfigsFromConnectionGraph
// (same as connectAllChains in implcommon.go) with optional ReconfigureLanesParams overlays.
...
// (RemoteLaneConfig.TestRouter false), no committee patches. To use TestRouter on a directed lane, set
...
// chain selector from chain ID + ChainFamily(), GetConnectionProfile(sel) on that impl.
func reconfigureLanesFromTopology(
	...
	if err := assertConnectionEntriesCoverSelectors(orderedSelectors, entries, selectors); err != nil {
		return err
	}

	chains, err := buildPartialChainConfigsFromConnectionGraph(topology, orderedSelectors, entries, params)
	if err != nil {
		return fmt.Errorf("reconfigure lanes: %w", err)
	}
