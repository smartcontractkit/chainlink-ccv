...
func NewV3ExtraArgs(finalityConfig uint16, gasLimit uint32, execAddr string, execArgs, tokenArgs []byte, ccvs []protocol.CCV) ([]byte, error) {
	...
	// Write tag
	buf.Write([]byte{0x30, 0x23, 0x26, 0xcb})
	...
	if len(ccvs) > 255 {
		return nil, fmt.Errorf("too many CCVs: %d (max 255)", len(ccvs))
	}
	...
	for i, ccv := range ccvs {
		...
		if len(ccv.Args) > 65535 {
			return nil, fmt.Errorf("CCV[%d] args too long: %d bytes (max 65535)", i, len(ccv.Args))
		}
	...
	if len(execArgs) > 65535 {
		return nil, fmt.Errorf("executor args too long: %d bytes (max 65535)", len(execArgs))
	}
	...
	if len(tokenArgs) > 65535 {
		return nil, fmt.Errorf("token args too long: %d bytes (max 65535)", len(tokenArgs))
	}
