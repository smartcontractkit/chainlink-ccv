...
var validAttestationResponse = []byte(`
	{
		"status": "complete",
...
func Test_HTTPClient_Get_Post(t *testing.T) {
	tt := []struct {
		...
		messageHash        protocol.Bytes32
		expectedError      error
		expectedResponse   protocol.ByteSlice
...
func Test_HTTPClient_GetInstance(t *testing.T) {
	...
	_, _, err = client1.Get(t.Context(), protocol.Bytes32{1, 2, 3}.String())
	require.NoError(t, err)

	timeoutCtx, cancel := context.WithTimeoutCause(t.Context(), 500*time.Millisecond, ErrTimeout)
	defer cancel()
	// This should return immediately with timeout error
	_, _, err = client2.Get(timeoutCtx, protocol.Bytes32{1, 2, 3}.String())
	require.Error(t, err)
	require.ErrorIs(t, err, ErrRateLimit)

	// This is different instance, should return success immediately
	_, _, err = client3.Get(t.Context(), protocol.Bytes32{1, 2, 3}.String())
	require.NoError(t, err)
}
