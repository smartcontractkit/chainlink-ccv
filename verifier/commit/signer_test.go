...
func TestNewSignerFromKeystore(t *testing.T) {
	...
	ctx := context.Background()
	signer, publicKey, err := NewSignerFromKeystore(ctx, ks, keyName)
	require.NoError(t, err)
	require.NotNil(t, signer)
	...
	require.NoError(t, err)
	expectedAddr := crypto.PubkeyToAddress(*expectedPubKey)
	require.Equal(t, expectedAddr[:], []byte(publicKey), "public key address should match")
	...
	r, s, signerAddr, err := protocol.DecodeSingleECDSASignature(signature)
	require.NoError(t, err)
	require.Equal(t, expectedAddr, signerAddr)
...
func TestNewSignerFromKeystore_KeyNotFound(t *testing.T) {
	...
	ctx := context.Background()
	_, _, err := NewSignerFromKeystore(ctx, ks, "non-existent-key")
	require.Error(t, err)
