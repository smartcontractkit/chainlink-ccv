...
// A task the gate cannot build a request for made no call, so it must not be counted as an
// endpoint failure. An operator paging on policy_endpoint_error would otherwise be woken by a
// verifier-side gap that leaves their endpoint entirely healthy.
func TestGatedVerifier_MissingReaderDetailsIsNotAnEndpointError(t *testing.T) {
	mon := newTransitionSpyMonitoring()
	gate, err := NewGatedVerifier(
		logger.Test(t), "committee-verifier-1", &stubVerifier{}, &stubChecker{}, mon, time.Second)
	require.NoError(t, err)

	task := newTask(msgID(1))
	task.MessageDetails = nil
	require.Len(t, gate.VerifyMessages(t.Context(), []vtypes.VerificationTask{task}), 1)

	require.Equal(t, []transition{{
		stage:   monitoring.MessageTransitionStagePolicy,
		outcome: monitoring.MessageTransitionOutcomePolicyUnavailable,
		reason:  monitoring.MessageTransitionReasonPolicyRequestInvalid,
	}}, mon.spy.recorded())
}

// An endpoint that actually failed keeps the endpoint reason, so the two stay distinguishable.
func TestGatedVerifier_EndpointFailureKeepsEndpointReason(t *testing.T) {
	...
	gate, err := NewGatedVerifier(
		logger.Test(t), "committee-verifier-1", &stubVerifier{}, checker, mon, time.Second)
	require.NoError(t, err)

	require.Len(t, gate.VerifyMessages(t.Context(), []vtypes.VerificationTask{newTask(msgID(1))}), 1)

	require.Equal(t, []transition{{
		stage:   monitoring.MessageTransitionStagePolicy,
		outcome: monitoring.MessageTransitionOutcomePolicyUnavailable,
		reason:  monitoring.MessageTransitionReasonPolicyEndpointError,
	}}, mon.spy.recorded())
}
...
// transitionSpy captures the message transitions the fake labeler discards.
...
func (s *transitionSpy) With(...string) vtypes.MetricLabeler { return s }

func (s *transitionSpy) IncrementMessageTransition(_ context.Context, stage, outcome, reason string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.calls = append(s.calls, transition{stage: stage, outcome: outcome, reason: reason})
}

func (s *transitionSpy) recorded() []transition {
	s.mu.Lock()
	defer s.mu.Unlock()
	return append([]transition(nil), s.calls...)
}

// transitionSpyMonitoring serves the transition spy in place of the fake labeler.
type transitionSpyMonitoring struct {
	*monitoring.FakeVerifierMonitoring
	spy *transitionSpy
}

func (m *transitionSpyMonitoring) Metrics() vtypes.MetricLabeler { return m.spy }

func newTransitionSpyMonitoring() *transitionSpyMonitoring {
	fake := monitoring.NewFakeVerifierMonitoring()
	return &transitionSpyMonitoring{
		FakeVerifierMonitoring: fake,
		spy:                    &transitionSpy{FakeVerifierMetricLabeler: fake.Fake},
	}
}
