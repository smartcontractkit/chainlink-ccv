...
type Config struct {
	...
	// local mode (bootstrap.WithLocalModeDefault) with no JD/DB/keystore and loads no bootstrap.Config;
	// its config file is already operator-provided (mounted, not JD-shipped), so this field already
