...
`verifier/generate-docs.sh` and regenerated by `just generate`, so read the YAML when you need the
authority and the rendered pages when you need the field tables.
