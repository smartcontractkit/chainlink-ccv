...
func initializeCoordinator(t *testing.T, verifierID string) *coordinatorTestSetup {
	...
	mockChainStatusManager.EXPECT().ReadChainStatus(mock.Anything, mock.Anything).Return(make(map[protocol.ChainSelector]*protocol.ChainStatusInfo), nil).Maybe()
	// Allow writes for chain status updates
	mockChainStatusManager.EXPECT().WriteChainStatus(mock.Anything, mock.Anything).Return(nil).Maybe()
