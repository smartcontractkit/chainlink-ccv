...
// SourceReaderServiceDB is a durable-queue-backed version of SourceReaderService.
// It replaces the in-memory batcher with a PostgreSQL-backed job queue so that
// ready verification tasks survive process restarts.
//
// The chain-reading, finality, curse and reorg logic is identical to
// SourceReaderService – only the output sink changes.
type SourceReaderServiceDB struct {
	...
	// Durable queue for ready tasks (consumed by TaskVerifierProcessorDB)
...
func NewSourceReaderServiceDB(
	...
	pollInterval := sourceCfg.PollInterval
	if pollInterval <= 0 {
	...
	for {
		...
		case <-ticker.C:
			if !r.disabled.Load() {
				...
				r.publishReadyMessages(ctx, latest, finalized)
			}
		}
	}
}
...
func (r *SourceReaderServiceDB) loadEvents(ctx context.Context, fromBlock *big.Int, latest *protocol.BlockHeader) ([]protocol.MessageSentEvent, error) {
	ranges := r.getBlockRanges(fromBlock.Uint64(), latest.Number)
...
//nolint:dupl // intentional duplicate of SourceReaderService.processEventCycle — in-memory version will be removed in a follow-up PR
func (r *SourceReaderServiceDB) processEventCycle(ctx context.Context, latest, finalized *protocol.BlockHeader) {
	...
	for _, event := range events {
		...
		if computedMessageID.String() != onchainMessageID {
			r.logger.Errorw("Message ID mismatch",
				"computed", computedMessageID.String(),
				"onchain", onchainMessageID)
			continue
		}
...
//nolint:dupl // intentional duplicate of SourceReaderService.addToPendingQueueHandleReorg — in-memory version will be removed in a follow-up PR
func (r *SourceReaderServiceDB) addToPendingQueueHandleReorg(tasks []VerificationTask, fromBlock *big.Int) {
	...
	// Remove tasks reorged out of pendingTasks
	...
	// Remove tasks reorged out of sentTasks
...
// publishReadyMessages checks which pending tasks have met finality requirements,
// publishes them to the durable task queue, and moves them to sentTasks.
func (r *SourceReaderServiceDB) publishReadyMessages(ctx context.Context, latest, finalized *protocol.BlockHeader) {
	r.logger.Infow("Checking for ready messages to publish",
		"latestBlock", latest.Number,
		"finalizedBlock", finalized.Number)
	...
	// Clean up sentTasks that are safely past finality (can never reorg)
	for msgID, task := range r.sentTasks {
		taskBlock := new(big.Int).SetUint64(task.BlockNumber)
	...
	if err := r.taskQueue.Publish(ctx, ready...); err != nil {
		r.logger.Errorw("Failed to publish ready tasks to job queue",
			"error", err,
			"count", len(ready))
	}
}

//nolint:dupl // intentional duplicate of SourceReaderService.isMessageReadyForVerification — in-memory version will be removed in a follow-up PR
