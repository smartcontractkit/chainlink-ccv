...
type IndexerMetrics struct {
	...
	// Scanner Metrics
	scannerPollingDurationMilliseconds metric.Int64Histogram
...
func InitMonitoring() (im *IndexerMetrics, err error) {
	im = &IndexerMetrics{}
	...
	im.scannerPollingDurationMilliseconds, err = beholder.GetMeter().Int64Histogram("indexer_scanner_polling_duration_milliseconds",
		metric.WithDescription("Total duration of polling the scanner"),
		metric.WithUnit("milliseconds"),
	)
	if err != nil {
		return nil, fmt.Errorf("failed to register scanner polling duration histogram: %w", err)
	}
...
func NewIndexerMetricLabeler(labeler metrics.Labeler, im *IndexerMetrics) *IndexerMetricLabeler {
...
func (c *IndexerMetricLabeler) With(keyValues ...string) *IndexerMetricLabeler {
...
func (c *IndexerMetricLabeler) RecordScannerPollingDuration(ctx context.Context, duration time.Duration) {
	otelLabels := beholder.OtelAttributes(c.Labels).AsStringAttributes()
	c.im.scannerPollingDurationMilliseconds.Record(ctx, duration.Milliseconds(), metric.WithAttributes(otelLabels...))
}
