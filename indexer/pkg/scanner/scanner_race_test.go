...
func TestScanner_Race_MultipleReadersConcurrent(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		results, err := setup.Storage.QueryCCVData(context.Background(), 0, time.Now().Unix()+1000, nil, nil, 1000, 0)
		if err != nil {
			return false
		}
...
func TestScanner_Race_ChannelOperations(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		results, err := setup.Storage.QueryCCVData(context.Background(), 0, time.Now().Unix()+1000, nil, nil, 100, 0)
		if err != nil {
			return false
		}
		return len(results) > 0
