...
func mustBuildEngine(ctx context.Context, needsDiscoveryReader bool) (*replay.Engine, func()) {
	...
	replayStore, err := replay.NewStoreFromConfig(ctx, lggr, pgCfg.URI, dbConfig, pgCfg.ConnMaxLifetime, pgCfg.ConnMaxIdleTime)
	if err != nil {
		lggr.Fatalf("Failed to create replay store: %v", err)
	}

	indexerStorage, err := storage.NewPostgresStorage(ctx, lggr, indexerMonitoring, pgCfg.URI, pg.DriverPostgres, dbConfig, pgCfg.ConnMaxLifetime, pgCfg.ConnMaxIdleTime)
	if err != nil {
...
func mustBuildStore(ctx context.Context) *replay.Store {
	...
	store, err := replay.NewStoreFromConfig(ctx, lggr, pgCfg.URI, dbConfig, pgCfg.ConnMaxLifetime, pgCfg.ConnMaxIdleTime)
	if err != nil {
