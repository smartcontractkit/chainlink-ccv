...
#
# The generated doccomments_gen.go files are deleted first so that deleting a documented type stays
# recoverable: the stale file left behind holds a method on a type that no longer exists, which
# breaks the package it sits in, and the generator imports every such package through its registry.
# Without the delete it could never build far enough to remove the file itself. Only files carrying
# this tool's generated-by header are touched, and the run rewrites every one it still reaches.
config-docs: ensure-go
    grep -rl --include='doccomments_gen.go' 'Code generated by github.com/smartcontractkit/chainlink-ccv/tools/configdoc' . | xargs rm -f
