...
// TestReceiptBlobDecodingErrors tests receipt blob decoding errors
		...
		},
		{
			...
			task: &VerificationTask{
				...
				ReceiptBlobs: []ReceiptWithBlob{
					{
						Issuer: UnknownAddress([]byte("different")),
						Blob:   []byte("blob"),
					},
				},
			},
			verifier:  UnknownAddress([]byte("target")),
