...
// - Network fee receipt (always at index length-1)
// - Executor receipt (always at index length-2)
// - Token receipts (at index length-3 if numTokenTransfers > 0).
func ParseReceiptStructure(receipts []ReceiptWithBlob, numCCVBlobs, numTokenTransfers int) (*ReceiptStructure, error) {
	...
	// Validate structure: CCVs + Tokens + 1 Executor + 1 Network fee = Total
	expectedReceipts := numCCVBlobs + numTokenTransfers + 2
	if len(receipts) != expectedReceipts {
		return nil, fmt.Errorf("unexpected receipt count: got %d, expected %d (CCVs=%d + Tokens=%d + Executor=1 + Network fee=1)",
	...
	// Extract executor receipt (always last at index length-2)
	result.ExecutorReceipt = receipts[len(receipts)-2]
	result.ExecutorAddress = result.ExecutorReceipt.Issuer

	// Extract token receipt (at index length-3 if present)
	if numTokenTransfers > 0 {
		result.TokenReceipts = append(result.TokenReceipts, receipts[len(receipts)-3])
	}
