...
func LoadConfigWithBlockchainInfos[T any](spec JobSpec) (*Configuration, accessors.Infos[T], error) {
	var decodeTarget cfgWithBlockchainInfos[T]
	md, err := toml.Decode(spec.ExecutorConfig, &decodeTarget)
	if err != nil {
		return nil, accessors.Infos[T]{}, fmt.Errorf("failed to decode executor config: %w", err)
	}
	if len(md.Undecoded()) > 0 {
		return nil, accessors.Infos[T]{}, fmt.Errorf("unknown fields in executor config: %v", md.Undecoded())
	}

	normalized, err := decodeTarget.GetNormalizedConfig()
	if err != nil {
		return nil, accessors.Infos[T]{}, fmt.Errorf("failed to normalize executor config: %w", err)
	}
