...
type ExecutorMetrics struct {
	...
	// broadcast/transmit attempt failure, labelled by cause
...
func InitMetrics() (*ExecutorMetrics, error) {
	...
	vm.broadcastAttemptErrorCounter, err = beholder.GetMeter().Int64Counter(
		...
		metric.WithDescription("Number of execute broadcast/transmit attempts that failed, labelled by cause. For cause=\"insufficient_funds\" the transmitter address was rejected by an RPC node for insufficient native funds; it increments on every retry while the address remains underfunded, so a sustained rate indicates the transmitter needs topping up."),
	)
	if err != nil {
