...
// configuration calls.
func LaneExpansion(registry *adapters.Registry) deployment.ChangeSetV2[LaneExpansionInput] {
	validate := func(e deployment.Environment, cfg LaneExpansionInput) error {
		return validateLaneInput(e, registry, cfg.SrcChainSelector, cfg.DestChainSelector)
	}

	apply := func(e deployment.Environment, cfg LaneExpansionInput) (deployment.ChangesetOutput, error) {
		return applyLaneConfig(e, registry, cfg.SrcChainSelector, cfg.DestChainSelector,
...
func validateLaneInput(
	...
	for _, sel := range []uint64{srcChainSelector, destChainSelector} {
		...
		if a.LaneConfig == nil {
			return fmt.Errorf("chain %d: no LaneConfig adapter registered", sel)
		}
	}
...
func applyLaneConfig(
	...
	for _, side := range sides {
		...
		a, err := registry.GetByChain(side.localSel)
		if err != nil {
