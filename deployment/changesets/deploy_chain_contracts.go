...
func DeployChainContracts(registry *adapters.Registry) deployment.ChangeSetV2[DeployChainContractsInput] {
	validate := func(e deployment.Environment, cfg DeployChainContractsInput) error {
		...
		for _, sel := range cfg.ChainSelectors {
			...
			if a.ChainContractsDeploy == nil {
				return fmt.Errorf("chain %d: no ChainContractsDeploy adapter registered", sel)
			}
		}
	...
	apply := func(e deployment.Environment, cfg DeployChainContractsInput) (deployment.ChangesetOutput, error) {
		...
		for _, sel := range cfg.ChainSelectors {
			...
			report, err := operations.ExecuteSequence(
				e.OperationsBundle,
				a.ChainContractsDeploy.DeployChainContracts(),
				e.BlockChains,
				input,
			)
