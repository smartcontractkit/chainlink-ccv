...
func newLaneTestRegistry() *adapters.Registry {
	r := adapters.GetRegistry()
	r.Register(chainsel.FamilyEVM, adapters.ChainAdapters{
		LaneConfig: &stubLaneConfigAdapter{},
	})
	return r
}
...
func TestLaneExpansion_Validation_MissingSrcChain(t *testing.T) {
	...
	cs := LaneExpansion(newLaneTestRegistry())
...
func TestLaneExpansion_Validation_NoLaneConfigAdapter(t *testing.T) {
	sel1 := chainsel.TEST_90000001.Selector
	sel2 := chainsel.TEST_90000002.Selector
	// Use an isolated registry with no LaneConfig adapter registered.
	...
	require.ErrorContains(t, err, "no LaneConfig adapter registered")
}
...
func TestPromoteLaneRouter_Validation_HappyPath(t *testing.T) {
	sel1 := chainsel.TEST_90000001.Selector
	sel2 := chainsel.TEST_90000002.Selector
	cs := PromoteLaneRouter(newLaneTestRegistry())
