...
- Add W3C-traceparent-based distributed tracing across the executor coordinator and verifier task pipeline, backed by a new `common/monitoring/tracing` package and a `Tracing()` method on `verifier.Monitoring` and `executor.Monitoring`.
...
- Affects: `verifier/pkg/*` (interfaces, storage, taskverifier, sourcereader, storagewriter, commit/utils), `executor/*` (coordinator, message_heap, monitoring), `common/monitoring/tracing` (new).

## AI Adapter Index

| Symbol | Kind | Search | Location | Section |
|---|---|---|---|---|
| `executor.NewCoordinator` | signature-changed | `NewCoordinator\(` | `executor/executor_coordinator.go:46` | [#coordinator-executorid-param](#coordinator-executorid-param) |
...
| `verifier.Monitoring.Tracing` / `executor.Monitoring.Tracing` | added | `\.Tracing\(\)` | `verifier/pkg/vtypes/interfaces.go:51`, `executor/interfaces.go:73` | [#tracing-interface](#tracing-interface) |
| `verifier.VerificationTask.TraceParent` / `.TraceContext` | added | `\.TraceParent\b\|\.TraceContext\b` | `verifier/pkg/vtypes/types.go:20-21` | [#tracing-interface](#tracing-interface) |
...
| `message_heap.MessageWithTimestamps.TraceContext` / `.DiscoveryContext` | added | `DiscoveryContext` | `executor/pkg/message_heap/message_heap.go:37-44` | [#tracing-interface](#tracing-interface) |
...
- **Why:** `executorID` names the per-message discovery span (`"executor.message@" + executorID`), attributing traces to the executor instance that discovered the message.
...
- **Distributed message tracing** — `common/monitoring/tracing.Tracing.StartMessageSpan(ctx, name, messageID, attrs...)` derives a deterministic OTel `TraceID` from the message ID when no real parent span exists, so all spans for a given message land in one trace even across process/queue boundaries. See `common/monitoring/tracing/tracing.go`.
  - Usage: executor coordinator opens a per-message "discovery" span at message discovery (`executor.message@<executorID>`) that stays open across retries via `DiscoveryContext` propagated through `message_heap.MessageWithTimestamps`, plus a per-attempt `executor.process_payload` span; verifier propagates `traceparent` through `VerificationTask`/`VerifierNodeResult` and injects/extracts it via `otel.GetTextMapPropagator()`.
