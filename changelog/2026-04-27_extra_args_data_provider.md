...
| `evm.SerializeAny2EVMMessageV1` | added | — | `build/devenv/evm/impl.go:710` | [New Features / Additions](#new-features--additions) |
| `evm.SerializeAny2EVMMessageV2` | added | — | `build/devenv/evm/impl.go:746` | [New Features / Additions](#new-features--additions) |
| `evm.SerializeAny2SVMMessageV1` | added | — | `build/devenv/evm/impl.go:810` | [New Features / Additions](#new-features--additions) |
...
7. **Migrate generic test scenarios.** `BasicMessageTestScenario` was deleted. Use the per-version helpers in `tests/composable/messaging/agnostic_chain_test.go`:
   ...
   - `Any2SVMMessageV1TestScenario` — type-asserts the source to `Any2SVMMessageV1` and serializes via `SerializeAny2SVMMessageV1`.
...
- **Optional source-side interfaces** that chain families can implement à la carte; generic test scenarios type-assert to them:
  ...
  - `Any2EVMMessageV1` — `SerializeAny2EVMMessageV1(opts any) ([]byte, error)`
  - `Any2EVMMessageV2` — `SerializeAny2EVMMessageV2(opts any) ([]byte, error)`
  - `Any2SVMMessageV1` — `SerializeAny2SVMMessageV1(opts any) ([]byte, error)`
...
- **EVM per-(version, family) serializers**, each type-asserting the provider:
  - `SerializeMessageV3ExtraArgs`, `SerializeAny2EVMMessageV1`, `SerializeAny2EVMMessageV2`, `SerializeAny2SVMMessageV1` (all in `build/devenv/evm/impl.go`). Each is exposed both as a free function (for the registry) and as a method on `*CCIP17EVM` (for satisfying the per-version source interfaces).
