...
* https://github.com/smartcontractkit/chainlink-ccv/pull/1022
...
5. [Step-by-step migration guide](#5-step-by-step-migration-guide)
   ...
   - [5.2 Remove the `ServiceFactory` type parameter](#52-remove-the-servicefactory-type-parameter)
...
**This means services should use `bootstrap.ServiceFactory`** as their factory type, not a custom app-config struct.
...
### 5.2 Remove the `ServiceFactory` type parameter
...
Your factory should implement `bootstrap.ServiceFactory`:

```go
type myFactory struct { /* fields for shutdown */ }

var _ bootstrap.ServiceFactory = (*myFactory)(nil)
...
func CreateStellarAccessorFactory(lggr logger.Logger, cfg chainaccess.GenericConfig) (chainaccess.AccessorFactory, error) {
