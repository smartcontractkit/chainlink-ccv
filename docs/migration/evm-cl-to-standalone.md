...
and tell Chainlink Labs; do not retry with a different value.
...
fallback, which is wrong for a slow chain), head-tracker persistence, and the RPC node set. The
`warnings` list names every setting your node config carries that standalone drops. Go through the
output with Chainlink Labs and either accept each deviation or correct it in the mounted config —
...
and fee-bumps far more aggressively than the node did on a slow chain. The fallback is loud: it is
logged at warn per chain at startup, and `ccv migrate inspect-config` flags it as
`txm_block_time_is_default` in the step 3 diff. Agree a per-chain value with Chainlink Labs before
the cutover; the [TXM v2 assessment](txm-v2-assessment.md) explains the fallback.
